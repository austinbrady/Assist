...
def create_user(username: str, password: str) -> Tuple[Optional[Dict], Optional[str]]:
    ...
    if username_trimmed:
        username_original = username_trimmed[0].upper() + username_trimmed[1:].lower()
    else:
        username_original = username_trimmed
    
    # Normalize username (lowercase for case-insensitive duplicate prevention)
    username_normalized = username_original.lower()
    ...
    # Log username creation for audit trail (use capitalized username)
    log_username(username_original, "created")
...
def generate_character_name(gender: Optional[str] = None) -> str:
    """
    Generate a unique character name based on gender
    Returns a name that sounds natural and fits the gender preference
    """
    import random
    
    male_names = [
        "Alex", "Jordan", "Casey", "Morgan", "Riley", "Avery", "Quinn", "Sage",
        "River", "Skyler", "Blake", "Cameron", "Dakota", "Emery", "Finley",
        "Hayden", "Jamie", "Kai", "Logan", "Noah", "Parker", "Reese", "Sam",
        "Taylor", "Tyler", "Zane", "Asher", "Caleb", "Ethan", "Lucas", "Mason",
        "Owen", "Ryan", "Sebastian", "Theo", "Wyatt", "Zachary", "Adrian", "Blake"
    ]
    ...
    female_names = [
        ...
        "River", "Skyler", "Blake", "Cameron", "Dakota", "Emery", "Finley",
        "Hayden", "Jamie", "Kai", "Logan", "Noah", "Parker", "Reese", "Sam",
        "Taylor", "Tyler", "Zane", "Aria", "Bella", "Chloe", "Diana", "Emma",
        "Fiona", "Grace", "Hannah", "Isla", "Jade", "Kira", "Luna", "Maya",
        "Nora", "Olivia", "Piper", "Quinn", "Ruby", "Sage", "Tessa", "Violet",
        "Willow", "Zoe", "Aurora", "Celeste"
    ]
    
    # If gender is specified, use that list; otherwise use a combined list
    if gender and gender.lower() == "male":
        names = male_names
    elif gender and gender.lower() == "female":
        names = female_names
    else:
        # Default to a mix if gender not specified
        names = male_names + female_names
    
    return random.choice(names)


def create_character(username: str, gender: Optional[str] = None, name: Optional[str] = None) -> Dict:
    """
    Create a new character for a user
    If name is not provided, generates one based on gender
    """
    users = load_users()
    ...
    if not name:
        name = generate_character_name(gender)
    
    # Create character
    character = {
        "id": f"char_{username}_{datetime.now().timestamp()}",
        "name": name,
    ...
    # Store characters in user data
    if "characters" not in users[username]:
        users[username]["characters"] = []
    
    # Check if this is the first character (make it primary)
    if len(users[username].get("characters", [])) == 0:
        character["is_primary"] = True
        users[username]["assistant_id"] = None  # Clear assistant_id when creating character
    
    users[username]["characters"].append(character)
    save_users(users)
    
    return character


def get_user_characters(username: str) -> List[Dict]:
    """Get all characters for a user"""
    users = load_users()
    if username in users:
        return users[username].get("characters", [])
    return []


def get_primary_character(username: str) -> Optional[Dict]:
    """Get the primary character for a user"""
    characters = get_user_characters(username)
    for char in characters:
        if char.get("is_primary", False):
            return char
    # If no primary, return first character
    if characters:
        return characters[0]
    return None


def set_primary_character(username: str, character_id: str) -> bool:
    """Set a character as primary"""
    users = load_users()
    if username not in users:
        return False
    
    characters = users[username].get("characters", [])
    for char in characters:
        if char["id"] == character_id:
            char["is_primary"] = True
            # Unset other primary characters
            for other_char in characters:
                if other_char["id"] != character_id:
                    other_char["is_primary"] = False
            save_users(users)
            return True
    return False
