...
# ============================================================
# Personal AI - Installation Script
# ============================================================
...
# Function to check and install Homebrew (AUTO-INSTALL)
check_homebrew() {
...
# Function to check and install Python (AUTO-INSTALL)
check_python() {
...
# Function to check and install Node.js (AUTO-INSTALL)
check_nodejs() {
...
# Function to check and install Ollama (AUTO-INSTALL)
check_ollama() {
...
install_dependencies() {
    ...
    mkdir -p "$SCRIPT_DIR/backend/songs"
    ...
    # Generate avatars (required Python script)
    echo -e "${YELLOW}Generating AI assistant avatars...${NC}"
    if python generate_avatars.py; then
        echo -e "${GREEN}✅ AI assistant avatars generated${NC}"
    else
        echo -e "${YELLOW}⚠️  Avatar generation had issues, but continuing...${NC}"
    fi
    cd "$SCRIPT_DIR"
...
main() {
    echo -e "${BLUE}============================================================${NC}"
    echo -e "${BLUE}  Personal AI - Installation${NC}"
    echo -e "${BLUE}============================================================${NC}"
    echo ""
    ...
    # Check/install Homebrew first (needed for other installs)
    check_homebrew
    echo ""
    ...
    echo "  1. Run ./START.sh to launch the GUI application"
    echo "  2. Use the toggle switch to start servers"
    echo "  3. Access Personal AI at http://localhost:7777"
    echo ""
    echo -e "${GREEN}🎉 Installation complete! You're ready to use Personal AI.${NC}"
    echo ""
}
